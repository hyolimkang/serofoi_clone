...
row_vector lambda_yexpo_fun (row_vector lambda,
 ...
 row_vector[Ymax] lambda_yexpo;
  ...
  int StartDec4 = Ymax - 40;
  int StartDec5 = 1;
  int EndDec1   = Ymax;
  int EndDec2   = StartDec1-1;
  int EndDec3   = StartDec2-1;
  int EndDec4   = StartDec3-1;
  int EndDec5   = StartDec4-1;
  
   for (j in StartDec1: EndDec1) lambda_yexpo[j] = lambda[5];
   for (k in StartDec2: EndDec2) lambda_yexpo[k] = lambda[4];
   for (l in StartDec3: EndDec3) lambda_yexpo[l] = lambda[3];
   for (m in StartDec4: EndDec4) lambda_yexpo[m] = lambda[2];
   for (n in StartDec5: EndDec5) lambda_yexpo[n] = lambda[1];
   return lambda_yexpo;
  }
}
