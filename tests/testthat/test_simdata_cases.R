test_that("simulated data", {
    ...
    model_constant <- run_seromodel(serodata = sim_data,
                                          foi_model = "constant")

    model_normal <- run_seromodel(serodata = sim_data,
                                      foi_model = "tv_normal")

    model_log <- run_seromodel(serodata = sim_data,
                                    foi_model = "tv_normal_log")

    model_plot_constant <- plot_seromodel(model_constant, size_text = 6, , max_lambda = max_lambda)
    model_plot_normal <- plot_seromodel(model_normal, size_text = 6, , max_lambda = max_lambda)
    model_plot_log <- plot_seromodel(model_log, size_text = 6, max_lambda = max_lambda)
    model_plot_arrange <- cowplot::plot_grid(model_plot_constant,
                                            model_plot_normal,
                                            model_plot_log,
                                            ncol = 3, nrow = 1)
    ...
    sim_foi_plot_log <- plot_foi(model_log, size_text = 10, max_lambda = max_lambda) +
      ggplot2::geom_point(data = data.frame(year = model_log$exposure_years,
                                            foi = sim_foi),
                          ggplot2::aes(year, foi))
    sim_foi_plot_arrange <- cowplot::plot_grid(sim_foi_plot_constant,
                                              sim_foi_plot_normal,
                                              sim_foi_plot_log,
                                              ncol = 1, nrow = 3)
    ...
    model_grouped_constant <- run_seromodel(serodata = sim_data_grouped,
                                                foi_model = "constant")
    model_grouped_normal <- run_seromodel(serodata = sim_data_grouped,
                                              foi_model = "tv_normal")
    model_grouped_log <- run_seromodel(serodata = sim_data_grouped,
                                            foi_model = "tv_normal_log")

    model_grouped_constant_plot <- plot_seromodel(model_grouped_constant, size_text = 6, max_lambda = max_lambda)
    model_grouped_normal_plot <- plot_seromodel(model_grouped_normal, size_text = 6, max_lambda = max_lambda)
    model_grouped_log_plot <- plot_seromodel(model_grouped_log, size_text = 6, max_lambda = max_lambda)
    model_grouped_plot_arrange <- cowplot::plot_grid(model_grouped_constant_plot,
    ...
    sim_foi_grouped_log_plot <- plot_foi(model_grouped_log, size_text = 10, max_lambda = max_lambda) +
      ggplot2::geom_point(data = data.frame(year = model_log$exposure_years,
                                            foi = sim_foi),
                          ggplot2::aes(year, foi))
    sim_foi_grouped_plot_arrange <- cowplot::plot_grid(sim_foi_grouped_constant_plot,
